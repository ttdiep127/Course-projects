    ...
    <div id="content">
        ...
        <div class="content_left_section_01">
            ...
            style="background: url('./images/welcome_title.jpg') no-repeat;"></div>
             <i style="color: red">
            <c:if test="${not empty mess_srq }"><p style="text-align: center; color: blue;">${mess_srq}</p></c:if>
            </i>
            <i style="color: red">
            <c:if test="${not empty rs_srq }"><p style="text-align: center; color: blue;">${rs_srq}</p></c:if>
            </i>
        <div style="text-align: center;padding-bottom : 15px;">
             <c:if test="${param.view_info_me eq 'true'}">
             <c:forEach items="${user_rs}" var = "lrs">
                <p style="color: red; text-align: center;"><a href="#"><strong>${lrs.getFull_name()}</strong> </a>muốn làm bạn với bạn.</p>
           <a href="${pageContext.request.contextPath}/SendRequest?rs_srq=ok&&i=${lrs.getId_user()}"> <button >Chấp nhận</button></a>
            <a href="${pageContext.request.contextPath}/SendRequest?rs_srq=del&&i=${lrs.getId_user()}"> <button >Từ chối</button></a>
            </c:forEach>
            </c:if>
            </div>
        	<p>
            ...
            <span class="show_info_colomn1"> </span> <span
            	...
            	<a href="${pageContext.request.contextPath}/SendMessage?w=${infoUser.getId_user()}&&n=${infoUser.getFull_name()}"><i class="fix_info">Nhắn tin</i></a> 
            	</c:if>
            	<c:if test="${relationship eq 'notyet'}">
            	<a href="${pageContext.request.contextPath}/SendRequest?i=${infoUser.getId_user()}&&search_user_name=${infoUser.getFull_name()}"><i class="fix_info">Gửi lời mời kết bạn</i></a>
            	</c:if>
            </span>
        	</p>
        </div>
